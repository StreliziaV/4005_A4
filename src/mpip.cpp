...
void slave(){
    ...
    while (cont) {
        ...
        if (count % 2 == 1) {
            omp_set_num_threads(n_omp_threads);
            #pragma omp parallel for
            for (int i = (my_begin_row_id * size); i < (my_end_row_id * size); i++) {
                update(odd_local_data, even_local_data, i);
            }
            omp_set_num_threads(n_omp_threads);
            #pragma omp parallel for
            for (int i = (my_begin_row_id * size); i < (my_end_row_id * size); i++) {
                maintain_fire(even_local_data, fire_area, i);
            }
            ...
            #pragma omp parallel for
            for (int i = (my_begin_row_id * size); i < (my_end_row_id * size); i++) {
                update(even_local_data, odd_local_data, i);
            }
            omp_set_num_threads(n_omp_threads);
            #pragma omp parallel for
            for (int i = (my_begin_row_id * size); i < (my_end_row_id * size); i++) {
                maintain_fire(odd_local_data, fire_area, i);
            }
